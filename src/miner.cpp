...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("YentenMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YentenMiner : ProcessBlock, block not accepted");
    }
...
{
    LogPrintf("YentenMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("yenten-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running YentenMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("YentenMiner terminated\n");
        throw;
    }
}
